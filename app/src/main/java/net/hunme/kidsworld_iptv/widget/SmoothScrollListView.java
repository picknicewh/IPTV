...
public class SmoothScrollListView extends ListView implements View.OnKeyListener, AbsListView.OnScrollListener {
    ...
    private ReviseFocusFollowListen followListen;
...
//				this.smoothScrollBy(-itemHeight, scrollDuration);
    ...
    /**
     * ListView 当前item获取的焦点后ListView在次重新获取焦点重新回到当前item（解决焦点就近原则）
     * 并且调用selectFromTop方法不让获得焦点的item置顶 使之归回原位！！！
     *
     * @param gainFocus
     * @param direction
    ...
    protected void onFocusChanged(boolean gainFocus, int direction, Rect previouslyFocusedRect) {
        lastSeletItem = getSelectedItemPosition();
        super.onFocusChanged(gainFocus, direction, previouslyFocusedRect);
        if (gainFocus) {
            setSelectionFromTop(lastSeletItem, getHeight() / 2);
        }
    }

    interface ReviseFocusFollowListen {
        void onReviseFocusFollow(View view, int position);
    }
}
